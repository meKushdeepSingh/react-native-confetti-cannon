...
class Explosion extends React.PureComponent<Props, State> {
  ...
  getItems = (prevColors: Array<string>): Array<Item> => {
    ...
    const newItems = Array(difference)
      .fill()
      .map((item, index): Item => ({
        leftDelta: randomValue(-1, 2),
        topDelta: randomValue(0, 1),
  ...
  start = (resume?: boolean = false) => {
    ...
    if (resume) {
      ...
      this.sequence = Animated.stagger(100, [
        ...
        {items.map((item: Item, index: number) => {
          const left = this.animation.interpolate({
            ...
            outputRange: [
              ...
              2 * (item.leftDelta * width),
            ],
          });
          const top = this.animation.interpolate({
            inputRange: [0, 0.2, 0.23, 1 + item.topDelta, 2],
            outputRange: [
              -origin.y,
              -item.topDelta * height,
              -item.topDelta * height,
              0,
              0,
            ],
          ...
          const rotateX = this.animation.interpolate({
            ...
            outputRange: [
              "0deg",
              `${item.speedDelta.rotateX * 360 * 5}deg`,
              `${item.speedDelta.rotateX * 360 * 15}deg`,
            ],
          });
          const rotateY = this.animation.interpolate({
            ...
            outputRange: [
              "0deg",
              `${item.speedDelta.rotateY * 360 * 3}deg`,
              `${item.speedDelta.rotateY * 360 * 10}deg`,
            ],
          });
          const rotateZ = this.animation.interpolate({
            ...
            outputRange: [
              "0deg",
              `${item.speedDelta.rotateZ * 360 * 1}deg`,
              `${item.speedDelta.rotateZ * 360 * 4}deg`,
            ],
          });
          const translateX = this.animation.interpolate({
            inputRange: [0, 0.2, 0.5, 2],
            outputRange: [0, -(item.swingDelta * 35), item.swingDelta * 35, 0],
          });
